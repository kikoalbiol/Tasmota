...
void GualiDetect(void) {
  ...
  AddLog(LOG_LEVEL_INFO, PSTR("GUALI GualiDetect at: 0x%02x "), guali_address);
  bmp_count++;
}
...
void (*const PROJECT_CMDMyProjectCommand[])(void) PROGMEM = {
    &PROJECT_CMDCmdSay_Hello, &PROJECT_CMDCmdSendMQTT, &PROJECT_CMDCmdHelp,
    &PROJECT_CMDCONFIG, &PROJECT_CMDCONFIG};
  ...
  if (XdrvMailbox.data_len > 0) {
    if (XdrvMailbox.payload != 0) {
      ...
      if (parser) {
        ...
        snprintf_P(PROJECT_CMDtopic, sizeof(PROJECT_CMDtopic),
                   PSTR("tasmota/RAWPROJECT_CMDCONFIG"));
        ...
        if (data_d.isArray()) {
          ...
          if (start_d) {
            ...
            const uint16_t start = start_d.getInt();
            const uint16_t start_length = length;
            AddLog(LOG_LEVEL_INFO, "START IS_INT");
            uint8_t buffsize = std::min<uint8_t>(array.size(), 512 - start);
            for (int i = 0; i < buffsize; i++) {
              uint16_t indx = i + start;
              buff_data[indx] = array[i].getInt();
              length = std::min<uint16_t>(500, indx); // Avoid overflow
            }

            snprintf_P(PROJECT_CMDtopic, sizeof(PROJECT_CMDtopic),
                       PSTR("tasmota/RANGE_GUALI1/%u/%u"), start_length,
                       length);
            done = true;
            auto commit = root[PSTR("commit")];
            if (commit) {
              if (commit.getBool()) {
                auto written = I2cWriteBuffer(guali_address, buff_data[0],
                                              buff_data + 1, length);
                snprintf_P(PROJECT_CMDtopic, sizeof(PROJECT_CMDtopic),
                           PSTR("tasmota/COMMIT_GUALI1/%u"), length);
                if (0 == written) {
                  AddLog(LOG_LEVEL_INFO,
                         PSTR("I2C configured with: %u written bytes"), length);
                } else {
      ...
      MqttPublishPayload(PROJECT_CMDtopic, PROJECT_CMDpayload,
                         strlen(PROJECT_CMDpayload), false);
    }
    if (done)
      ResponseCmndDone();
  }
...
void PROJECT_CMDCmdSendMQTT(void) {
  ...
  snprintf_P(PROJECT_CMDtopic, sizeof(PROJECT_CMDtopic),
             PSTR("tasmota/USE_IFIC_ADC_GUALI1"));

  snprintf_P(
      PROJECT_CMDpayload, sizeof(PROJECT_CMDpayload),
...
void PROJECT_CMDCmdHelp(void) {
  AddLog(LOG_LEVEL_INFO, PSTR("Help: Accepted commands - Say_Hello, SendMQTT, "
                              "Help, ConfigMatrix, ConfigRawMatrix"));
  ResponseCmndDone();
}
...
bool Xdrv101(uint32_t function) {
  ...
  if (FUNC_INIT == function) {
    ...
    case FUNC_COMMAND:
      ...
      result = DecodeCommand(PROJECT_CMDMyProjectCommands,
                             PROJECT_CMDMyProjectCommand);
      break;
    }
  }
